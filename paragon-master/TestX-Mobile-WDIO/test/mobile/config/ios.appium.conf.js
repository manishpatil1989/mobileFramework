"use strict";
global.env = 'ios';
global._checkDeviceRunning = false;
...
const deviceInfoList = require('./ios.device.conf').deviceInfoList;
...
let TFS_DOMAIN = 'TMS URL'
global.DMA_APP_NAME = "com.pcloudy.appiumdemo";
  ...
  //  maxInstancesPerCapability: 1,
    ...
    capabilities: [
      {
        ...
        noReset: true,
        wdaLocalPort: deviceCapability.wdaLocalPort,
        ...
        //clearSystemFi les: true,
        app: join(process.cwd(), './apps/ios/TestApp.app'),
        connectHardwareKeyboard: false,
        clearSystemFiles: true,
        autoAcceptAlerts: true,
        locationServicesEnabled: false,
        launchTimeout: 60000,
        permissions: '{"com.pcloudy.appiumdemo": { "photos": "YES", "camera": "YES", "location": "always", "notifications": "YES", "userTracking": "YES"}}'
        //browserName: 'firefox',
    ...
    services: [
        ['native-app-compare', {
          ...
          baselineFolder: join(process.cwd(), './test/mobile/visualBaseline/ios'),
          screenshotPath: join(process.cwd(), './test/mobile/reports/ios-visual'),
    ...
    ...((global._TESTFEEDER) ? [] :
      [['appium', {
        ...
        [join(process.cwd(), './test/mobile/service/testCounter/index.js'), {
          allureResultsDir : './test/mobile/reports/allure-results-ios/'
        }],
    ...
    reporters: [
      'spec',
      ['json', {
        outputDir: './test/mobile/reports/json-results/ios/'
      }],
      ...((argv.reporter && argv.reporter.noVideo) ? [] :
        [[video, {
          ...
          outputDir: './test/mobile/reports/video-ios/',
          //addExcludedActions: ['displayed', 'enabled']
        }]]),
     ['allure', {
        outputDir: './test/mobile/reports/allure-results-ios/',
        disableWebdriverStepsReporting: true,
        disableWebdriverScreenshotsReporting: !takesScreenShotUponFailure,
        useCucumberStepReporter: true,
        issueLinkTemplate: TFS_DOMAIN,
        tmsLinkTemplate: TFS_DOMAIN, 
      }],     
    ],
    ...
    onPrepare: function (config, capabilities) {
      ...
      return new Promise((resolve,reject) => {
        global._checkDeviceRunning = true;
          emulatorHelper.getEmulatorState().then((EMUALTORS) => {
            if(!EMUALTORS.includes(deviceCapability.platformVersion)) {
             ...
             */
            let deviceToVerify = [capabilities[0].deviceName + ' (' +capabilities[0].platformVersion+')'];
            emulatorHelper.getEmulatorID().then((devices) => {
              if(devices.length) {
                emulatorHelper.getEmulatorToClose(devices, deviceToVerify).then((emulatorID) => {
                  if(emulatorID.length && !ignoreDevice) {
                    console.log('\x1b[33m%s\x1b[0m', 'ERROR: Device ' + deviceToVerify + ' already in use');
                    return reject();
                  } else {
                    return resolve();
                  }
    ...
    afterScenario: function (world, result, context) {
       ...
       reporter.addEnvironment('DEVICE :', global.deviceCapability.deviceName);
    ...
    onComplete: function(exitCode, config, capabilities, results) {
      ...
      if(!global._TESTFEEDER) {
        ...
        return new Promise((resolve, reject) => {
          let deviceToClose = [capabilities[0].deviceName + ' (' +capabilities[0].platformVersion+')'];
          emulatorHelper.getEmulatorID().then((devices) => {
            if(devices) {
              emulatorHelper.getEmulatorToClose(devices, deviceToClose).then((emulatorID) => {
